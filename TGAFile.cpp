		...
		case 24:
			...
			if( bit5TopToBottom != 0 )//Origen arriba
			{
				...
				}else//Origen izquierda
				{
					...
					{
						Stream.read((char*)&m_ArregloColores[i*3+2], sizeof(unsigned char));//B -> Azul
						Stream.read((char*)&m_ArregloColores[i*3+1], sizeof(unsigned char));//G -> Verde
						Stream.read((char*)&m_ArregloColores[i*3]  , sizeof(unsigned char));//R -> Rojo
					}
				}
			}else//Origen abajo
			{
		...
		case 32:
			...
			if( bit5TopToBottom != 0 )//Origen arriba
			{
				...
				}else
				{
					for(unsigned int i=0; i < m_Width*m_Height ; ++i)
					{
						Stream.read((char*)&m_ArregloColores[i*4+2], sizeof(unsigned char));//B -> Azul
						Stream.read((char*)&m_ArregloColores[i*4+1], sizeof(unsigned char));//G -> Verde
						Stream.read((char*)&m_ArregloColores[i*4]  , sizeof(unsigned char));//R -> Rojo
						Stream.read((char*)&m_ArregloColores[i*4+3], sizeof(unsigned char));//A -> Alpha
					}
				}
			}else//Origen abajo
			{
